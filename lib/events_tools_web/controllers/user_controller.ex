defmodule EventsToolsWeb.UserController do
  ...
  def new(conn, _params, current_user, claims) do
    changeset = Accounts.change_user(%EventsTools.Accounts.User{})
    user = %{roles: [], status: []}
    roles_all = Repo.all(EventsTools.Accounts.Roles)
    status_all = Repo.all(EventsTools.Status.Status)
